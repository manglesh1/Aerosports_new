...
export async function generateMetadata() {
  const meta = await generateMetadataLib({ location: "", page: "team-celebrations" });
  if (meta.title === "AeroSports Trampoline Park") {
    meta.title = "Team Celebrations | AeroSports Parks Canada";
    meta.openGraph.title = "Team Celebrations | AeroSports Parks Canada";
    meta.twitter.title = "Team Celebrations | AeroSports Parks Canada";
  }
  if (meta.description === "Fun for all ages at AeroSports!") {
    meta.description = "Celebrate your team's big wins at AeroSports Parks. End-of-season parties, tournament celebrations, and team outings for sports teams and local clubs.";
...
export default async function TeamCelebrationsPage() {
  const pageData = await fetchPageData("", "team-celebrations");
  ...
  return (
    <main className="hv2" style={{ background: "#fff", minHeight: "100vh" }}>
      ...
      <MotionImage
        pageData={{
          ...pageData,
          title: pageData?.title || 'Team Celebrations',
          smalltext: pageData?.subtitle || pageData?.smalltext || 'Celebrate your team\'s big wins with active parties and team outings.',
        }}
      />
            ...
            <>
              <div style={{ maxWidth: 900, margin: "0 auto 4rem" }}>
                ...
                <p style={{ fontSize: 16, lineHeight: 1.8, color: "#555" }}>
                  Whether your team just won the championship or finished a great season, AeroSports is
                  the perfect place to celebrate. Our parks offer high-energy fun for athletes of all ages,
                  with space for the whole team to jump, climb, flip, and compete in a safe, supervised environment.
                </p>
              </div>
              ...
              <div style={{ maxWidth: 900, margin: "0 auto 4rem" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 20 }}>
                  {[
                    { icon: "⚽", title: "Sports Teams", desc: "Hockey, soccer, basketball, baseball — celebrate your season at AeroSports." },
                    { icon: "💃", title: "Dance & Gymnastics", desc: "Recital wrap parties and end-of-year celebrations for dance studios and gyms." },
                    { icon: "🏊", title: "Swim & Track Clubs", desc: "Reward your athletes with an action-packed outing after the big meet." },
                    { icon: "🎭", title: "Clubs & Organizations", desc: "Scouts, guides, martial arts clubs — any group looking to celebrate together." },
                    { icon: "🍕", title: "Food & Party Rooms", desc: "Add pizza, drinks, and a private party room for your team celebration." },
                    { icon: "🏅", title: "Awards & Recognition", desc: "Hand out trophies and awards in our private event spaces before hitting the attractions." },
                  ].map((item, i) => (
                    <div
                      key={i}
                      style={{
              ...
              <div style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
                <h2 className="hv2-section-h2" style={{ marginBottom: 16 }}>
                  Book Your Team Celebration
                </h2>
                <p style={{ fontSize: 16, color: "#555", marginBottom: 32 }}>
                  Choose your nearest park to book a team party package.
                </p>
              </div>
            </>
          )}
          ...
          <div style={{ maxWidth: 1200, margin: "3rem auto 0" }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 20 }}>
              {locations.map((loc, i) => (
                <Link
                  key={i}
                  href={`/${loc.locations}/groups-events`}
                  style={{
                    display: "block",
                    background: "#fff",
                    borderRadius: 16,
