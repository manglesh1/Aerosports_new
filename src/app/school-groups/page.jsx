...
export async function generateMetadata() {
  const meta = await generateMetadataLib({ location: "", page: "school-groups" });
  if (meta.title === "AeroSports Trampoline Park") {
    meta.title = "School Groups & Field Trips | AeroSports Parks Canada";
    meta.openGraph.title = "School Groups & Field Trips | AeroSports Parks Canada";
    meta.twitter.title = "School Groups & Field Trips | AeroSports Parks Canada";
  }
  if (meta.description === "Fun for all ages at AeroSports!") {
    meta.description = "Plan an unforgettable school field trip at AeroSports Parks. Educational, active group outings that students and teachers love. Book your school group today.";
...
export default async function SchoolGroupsPage() {
  const pageData = await fetchPageData("", "school-groups");
  ...
  return (
    <main className="hv2" style={{ background: "#fff", minHeight: "100vh" }}>
      ...
      <MotionImage
        pageData={{
          ...pageData,
          title: pageData?.title || 'School Groups & Field Trips',
          smalltext: pageData?.subtitle || pageData?.smalltext || 'Plan an unforgettable school outing with active group experiences students and teachers love.',
        }}
      />
            ...
            <>
              ...
              <div style={{ maxWidth: 900, margin: "0 auto 4rem" }}>
                ...
                <h2 className="hv2-section-h2" style={{ marginTop: 12, marginBottom: 16 }}>
                  The Perfect Active Field Trip
                </h2>
                <p style={{ fontSize: 16, lineHeight: 1.8, color: "#555" }}>
                  AeroSports provides a safe, supervised environment where students can burn off energy,
                  build teamwork skills, and have a blast — all under one roof. Our school group packages
                  are designed to make planning easy for teachers and administrators while keeping students
                  engaged and active.
                </p>
              </div>
              ...
              <div style={{ maxWidth: 900, margin: "0 auto 4rem" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 20 }}>
                  {[
                    { icon: "🎯", title: "Full Attraction Access", desc: "Students enjoy trampolines, ninja courses, climbing walls, and more." },
                    { icon: "👨‍🏫", title: "Supervised Sessions", desc: "Trained staff monitor all activity areas for a safe, fun experience." },
                    { icon: "🍕", title: "Food Options Available", desc: "Add pizza, drinks, and snacks to keep everyone fueled." },
                    { icon: "📋", title: "Easy Waiver Process", desc: "Digital waivers make paperwork simple for teachers and parents." },
                    { icon: "💰", title: "Group Pricing", desc: "Special discounted rates for school groups of all sizes." },
                    { icon: "📅", title: "Flexible Scheduling", desc: "Book weekday mornings or afternoons to fit your school calendar." },
                  ].map((item, i) => (
                    <div
                      key={i}
                      style={{
              ...
              <div style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
                ...
                <p style={{ fontSize: 16, color: "#555", marginBottom: 32 }}>
                  Choose your nearest park to check availability and book your school group outing.
                </p>
              </div>
            </>
          )}
          ...
          <div style={{ maxWidth: 1200, margin: "3rem auto 0" }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 20 }}>
              {locations.map((loc, i) => (
                <Link
                  key={i}
                  href={`/${loc.locations}/groups-events/school-groups`}
                  style={{
                    display: "block",
                    background: "#fff",
                    borderRadius: 16,
