        ...
        {items.map((src, idx) => (
          <button
            ...
            className={`w-14 h-14 lg:w-20 lg:h-20 bg-cover bg-center rounded-xl border-2 transition-all duration-200 ${
              selected === src
                ? "border-[#B7E600] ring-2 ring-[#B7E600]/40 scale-105"
...
function InvitationForm({ details, onChange, checkInTime, onTimeChange }) {
  const inputClass =
    "mt-1 w-full p-2.5 bg-black/30 border border-white/20 text-white placeholder-white/40 rounded-lg focus:border-[#B7E600] focus:ring-1 focus:ring-[#B7E600]/30 focus:outline-none transition-colors";
  const labelClass = "block text-sm font-semibold text-gray-300 mb-1";
  const selectClass =
    "p-2.5 bg-black/30 border border-white/20 text-white rounded-lg focus:border-[#B7E600] focus:outline-none appearance-none";
...
function InvitationPreview({ cardRef, bgImage, fgImage, details, checkInTime, onDownload }) {
  return (
    <div className="w-full h-[600px] lg:h-[700px] rounded-2xl shadow-2xl relative overflow-hidden">
      ...
      <button
        onClick={onDownload}
        className="absolute top-3 right-3 bg-[#B7E600] hover:bg-[#e00e47] text-white p-3 rounded-full shadow-lg transition-all duration-200 hover:scale-110 z-10"
...
export default function InvitationsPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-950 to-black">
      {/* Header banner */}
      <div className="bg-gradient-to-r from-[#B7E600]/20 to-[#B7E600]/10 border-b border-white/10 px-6 py-8 text-center">
        <span className="inline-block mb-3 px-4 py-1.5 border border-[#B7E600] rounded-full text-[#B7E600] text-xs font-bold uppercase tracking-widest">
          Birthday Parties
        </span>
        <h1 className="text-3xl md:text-4xl font-black text-white uppercase tracking-wide">
          Create Your <span className="text-[#B7E600]">Party Invitation</span>
        </h1>
