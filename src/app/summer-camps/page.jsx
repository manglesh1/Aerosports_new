...
export async function generateMetadata() {
  const meta = await generateMetadataLib({ location: "", page: "summer-camps" });
  if (meta.title === "AeroSports Trampoline Park") {
    meta.title = "Summer Camps | AeroSports Parks Canada";
    meta.openGraph.title = "Summer Camps | AeroSports Parks Canada";
    meta.twitter.title = "Summer Camps | AeroSports Parks Canada";
  }
  if (meta.description === "Fun for all ages at AeroSports!") {
    meta.description = "Keep kids active and entertained all summer at AeroSports Parks. Indoor summer camp programs with trampolines, ninja courses, games, and more across Ontario.";
...
export default async function SummerCampsPage() {
  const pageData = await fetchPageData("", "summer-camps");
  ...
  return (
    <main className="hv2" style={{ background: "#fff", minHeight: "100vh" }}>
      ...
      <MotionImage
        pageData={{
          ...pageData,
          title: pageData?.title || 'Summer Camps',
          smalltext: pageData?.subtitle || pageData?.smalltext || 'Keep kids active and entertained all summer with trampolines, ninja courses, games, and more.',
        }}
      />
          ...
          {pageData?.section1 ? (
            ...
            <>
              <div style={{ maxWidth: 900, margin: "0 auto 4rem" }}>
                ...
                >
                  Why AeroSports Camps
                </span>
                <h2 className="hv2-section-h2" style={{ marginTop: 12, marginBottom: 16 }}>
                  The Ultimate Indoor Camp Experience
                </h2>
                <p style={{ fontSize: 16, lineHeight: 1.8, color: "#555" }}>
                  Rain or shine, AeroSports summer camps keep kids moving, laughing, and making friends.
                  Our indoor facilities mean no weather cancellations, and our trained staff ensure every
                  camper has a safe, action-packed day. From trampolines and ninja courses to team games
                  and free play, every day is a new adventure.
                </p>
              </div>
              ...
              <div style={{ maxWidth: 900, margin: "0 auto 4rem" }}>
                <span
                  className="hv2-section-tag"
                  style={{ background: "rgba(255,45,135,0.15)", color: "var(--hv2-lime-dark)" }}
                >
                  Camp Highlights
                </span>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 20 }}>
                  {[
                    { icon: "🤸", title: "Non-Stop Action", desc: "Trampolines, dodgeball, ninja courses, climbing walls, and more every single day." },
                    { icon: "🌧️", title: "Weather-Proof", desc: "100% indoor facilities mean camp never gets rained out or cancelled." },
                    { icon: "👫", title: "New Friendships", desc: "Team activities and group games help campers make lasting connections." },
                    { icon: "🛡️", title: "Trained Staff", desc: "Experienced camp counselors focused on safety, fun, and inclusion." },
                    { icon: "🍎", title: "Snacks & Lunch Options", desc: "Add food packages or bring your own — we have a dedicated eating area." },
                    { icon: "📆", title: "Flexible Registration", desc: "Sign up by the week or the whole summer. Drop-off and pick-up times that work for parents." },
                  ].map((item, i) => (
                    <div
                      key={i}
                      style={{
              ...
              <div style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
                ...
                <p style={{ fontSize: 16, color: "#555", marginBottom: 32 }}>
                  Choose your nearest park to see camp schedules, pricing, and availability.
                </p>
              </div>
            </>
          )}
          ...
          <div style={{ maxWidth: 1200, margin: "3rem auto 0" }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 20 }}>
              {locations.map((loc, i) => (
                <Link
                  key={i}
                  href={`/${loc.locations}/programs/summer-camps`}
                  style={{
                    display: "block",
                    background: "#fff",
                    borderRadius: 16,
